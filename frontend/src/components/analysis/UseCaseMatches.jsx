...
const RelevanceScore = ({ score }) => {
  ...
  const percentage = typeof score === 'number' ? score : parseInt(score, 10);
...
const UseCaseMatches = ({ matches }) => {
  ...
  const sortedMatches = [...matches].sort((a, b) => {
    const scoreA = typeof a.relevanceScore === 'number' ? a.relevanceScore : parseInt(a.relevanceScore, 10);
    const scoreB = typeof b.relevanceScore === 'number' ? b.relevanceScore : parseInt(b.relevanceScore, 10);
        ...
        <div key={index} className="bg-white border rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow">
          ...
          <div className="p-4">
            ...
            <div className="flex justify-end mt-4">
              <Link 
                to={`/roi-calculator?useCase=${match.useCase}&relevanceScore=${match.relevanceScore}`}
